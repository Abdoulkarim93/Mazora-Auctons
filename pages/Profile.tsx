                            ...
                            {completedItems.length > 0 ? completedItems.map((item: any) => (
                                <div key={item.id} className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-12 items-start animate-fade-in">
                                    <div className="bg-white rounded-[2.5rem] border border-gray-100 p-6 md:p-8 flex gap-6 shadow-sm group hover:shadow-xl transition-all">
                                        <div className="w-20 h-20 md:w-32 md:h-32 rounded-[1.5rem] md:rounded-[2rem] overflow-hidden shrink-0 shadow-lg"><LazyImage src={user.role === 'seller' ? item.imageUrl : item.auctionImage} alt="Completed" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" /></div>
                                        <div className="flex-1 min-w-0 flex flex-col justify-center"><span className="bg-emerald-100 text-emerald-700 text-[8px] font-black px-2 py-0.5 rounded uppercase mb-2 inline-block self-start">İŞLEM BAŞLADI ✅</span><h4 className="font-display font-black text-lg md:text-2xl text-gray-900 uppercase italic truncate mb-2 leading-tight tracking-tighter">{user.role === 'seller' ? item.title : item.auctionTitle}</h4><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{new Date(item.timestamp || item.createdAt).toLocaleDateString()}</p></div>
                                    </div>
                                    <div className="space-y-6">
                                        <TrustStepper item={item} role={user.role as 'buyer' | 'seller'} />
                                        <FeeBreakdown baseAmount={user.role === 'seller' ? (item.reservePrice || item.startPrice) : item.myBid} condition={item.condition} role={user.role as 'buyer' | 'seller'} isBoosted={item.isBoosted} />
                                    </div>
                                </div>
